...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['pvda', 'kpd', 'ddp', 'z', 'dvp', 'dnvp', 'nsdap', 'other'];
    }
    if (!partyColors) {
        partyColors = {'pvda': '#E3000F', 'kpd': '#8B0000', 'ddp': '#DCCA4A', 'z': '#000', 'dvp': '#D5AC27', 'dnvp': '#3f7bc1', 'nsdap': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'pvda': 'pvda', 'kpd': 'KPD', 'ddp': 'DDP', 'z': 'Z + BVP', 'dvp': 'DVP', 'dnvp': 'DNVP', 'nsdap': 'NSDAP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxpvda = d3.max(data, d => d.pvda);
          const maxNSDAP = d3.max(data, d => d.nsdap);
          dataMax = maxpvda >= maxNSDAP ? maxpvda + 10 : maxNSDAP + 10;
          dataMin = 0;
      }
